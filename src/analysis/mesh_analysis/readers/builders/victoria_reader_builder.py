...
class VictoriaQueryBuilder(BaseModel):
    ...
    def with_order_by(self, order_by) -> Self:
        order_by = order_by.strip()
        if order_by.startswith("|"):
            order_by = order_by[1:].strip()
        if not order_by.lower().startswith("order by"):
    ...
    def _build(self, query) -> dict:
        ...
        if self.order_by:
            full_query += "|" + self.order_by
        if self.uniq_by:
            full_query += "|" + self.uniq_by
        return {"query": full_query}

    def build_query_config(self) -> dict:
        queries = [self._build(query) for query in self.queries]
        if len(queries) == 1:
            queries = queries[0]
        return {"url": self.url, "headers": {"Content-Type": "application/json"}, "params": queries}
...
class VictoriaReaderBuilder(BaseModel):
    ...
    def _query_builder(
        self, *, uniq_by: Optional[str] = None, order_by: Optional[str] = None
    ) -> VictoriaQueryBuilder:
        query_builder = (
            VictoriaQueryBuilder().with_url(self.kwargs["url"]).with_filters(self.kwargs)
        )
        if uniq_by:
    ...
    def build_with_pod_identifier(
        ...
        *,
        ...
        query_builder = self._query_builder(uniq_by=uniq_by, order_by=order_by).with_pod_identifier(
            pod_name
        )
