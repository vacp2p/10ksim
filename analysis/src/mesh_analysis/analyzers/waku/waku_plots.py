...
def plot_message_distribution_libp2pmix(
...
def plot_message_latency_distributions(
    ...
    ax = sns.boxplot(x="source", y="latency", data=merged, showfliers=False)
...
def plot_message_distribution(
    received_summary_path: Path, plot_title: str, dump_path: Path
) -> Result[None, str]:
    """
    ...
    time_ranges = df.groupby(level="msg_hash").apply(
        lambda x: (
            x.index.get_level_values("timestamp").max()
            - x.index.get_level_values("timestamp").min()
        ).total_seconds()
    )
...
def plot_message_distribution_mixnet(
    received_summary_path: Path, sent_summary_path: Path, plot_title: str, dump_path: Path
) -> Result[None, str]:
    if not received_summary_path.exists():
        error = f"Received summary file {received_summary_path} does not exist"
        logger.error(error)
    ...
    for (shard, msg_hash), group in df_sent.groupby(["shard", "msg_hash"]):
        ...
        if received_group.iloc[-1]["receiver_peer_id"] != group.iloc[-1]["receiver_peer_id"]:
            logger.error(
                f"Final mismatch at shard {shard}, msg_hash {msg_hash}: "
                f"{received_group.iloc[-1]['receiver_peer_id']} != {group.iloc[-1]['sender_peer_id']}"
            )
    ###################

    latest_received = (
        df_received.groupby(level="msg_hash")
        .apply(lambda x: x.index.get_level_values("timestamp").max())
        .rename("last_received")
    )

    sent_times = (
        df_sent.groupby("msg_hash")
        .apply(lambda x: x.index.get_level_values("timestamp").min())
        .rename("injected_at")
    )
...
def check_time_to_reach_value_plot(
    ...
    df_1 = pd.DataFrame(
        {"Time to Target": time_to_target_1, "Dataset": [file_path_1.name] * len(time_to_target_1)}
    )
