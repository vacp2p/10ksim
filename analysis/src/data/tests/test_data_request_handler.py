...
class TestDataRequestHandler(unittest.TestCase):
    ...
    def test_create_dataframe_from_request(self):
        raw_data = {
            "data": {
                "result": [
                    {
                        "metric": {"instance": "instance1", "job": "job1"},
                        "values": [["1", 2], ["2", 4]],
                    },
                    {
                        "metric": {"instance": "instance2", "job": "job2"},
                        "values": [["1", 3], ["2", 6]],
                    },
                ]
            }
    ...
    def test__extract_column_name_from_result(self):
        ...
        pod_result = {"metric": {"instance": "instance1", "job": "job1"}}
        self.assertEqual(
            drh._extract_column_name_from_result("job-instance", pod_result), "job1_instance1"
        )

    def test__extract_column_name_from_result_multiple(self):
        drh = DataRequestHandler({})
        pod_result = {"metric": {"instance": "instance1", "job": "job1", "cluster": "cluster1"}}
        self.assertEqual(
            drh._extract_column_name_from_result("job-instance-cluster", pod_result),
            "job1_instance1_cluster1",
        )
    ...
    def test__create_pod_df(self):
        ...
        self.assertIsNone(
            pd.testing.assert_frame_equal(drh._create_pod_df(column_name, values), expected_df)
        )
    ...
    def test__sort_dataframe_columns(self):
        ...
        self.assertEqual(
            drh.dataframe.columns.tolist(), ["bootstrap-1_in", "bootstrap-2_in", "bootstrap-3_in"]
        )
