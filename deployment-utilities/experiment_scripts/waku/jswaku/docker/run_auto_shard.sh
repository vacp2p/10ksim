...
nwaku_args=(--lightpush=true --relay=true --max-connections=500 --rest=true --rest-admin=true --rest-address=0.0.0.0 --discv5-discovery=true --discv5-enr-auto-update=True --log-level=INFO --metrics-server=True --metrics-server-address=0.0.0.0  --cluster-id=2 --websocket-support=true --num-shards-in-network=$num_shards)
...
nwaku_relay_args=(--relay=true --max-connections=500 --rest=true --rest-admin=true --rest-address=0.0.0.0 --discv5-discovery=true --discv5-enr-auto-update=True --log-level=INFO --metrics-server=True --metrics-server-address=0.0.0.0  --cluster-id=2 --websocket-support=true --staticnode=$addrs1 --num-shards-in-network=$num_shards)
...
curl -X POST http://$node:${jswaku_external_port}/lightpush/v3/message \
  -H "Content-Type: application/json" \
  -d '{ "pubsubTopic":"",   "message": {"contentTopic": "/test/1/cross-network/proto", "payload" : "[1, 2, 3]"}}'
