...
Make sure you do not create a python virtual in this folder. `make format` and the `registry.py` scan will raise errors.
...
Experiments are gathered by `registry.py`, which scans files looking for the `@experiment(...)` decorator.
...
-  Called in `main.py/run_experiment` with `args` and `values_yaml` from CLI args and `api_client` created with the kubeconfig value from `--config`.
