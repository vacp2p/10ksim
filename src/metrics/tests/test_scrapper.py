...
class TestScrapper(unittest.TestCase):
    ...
    def test_query_and_dump_metrics_single(self, mock_get_query_data: MagicMock):
        ...
        test_scrapper = scrapper.Scrapper(None, "http://myurl:9090/api/v1/",
                                          file_path)

        data = {'data': {
            'result': [{'metric': {'instance': 'nodes-1'}, 'values': [[1, 5], [2, 5], [3, 5],
                                                                      [4, 5], [5, 5]]}]}}
    ...
    def test_query_and_dump_metrics_multiple_column_unordered(self, mock_get_query_data: MagicMock):
        ...
        data = {'data': {
            'result': [{'metric': {'instance': 'nodes-2'}, 'values': [[1, 6], [2, 6], [3, 6],
                                                                      [4, 6], [5, 6]]},
    ...
    def test_query_and_dump_metrics_multiple_fail(self, mock_dump: MagicMock,
                                                  mock_get_query_data: MagicMock):
        file_path = os.path.join(self.current_directory, 'test_scrape.yaml')
        test_scrapper = scrapper.Scrapper(None, "http://myurl:9090/api/v1/",
                                          file_path)
