...
class StatusgoMessageLogAnalyzer:
    def __init__(
        ...
        timestamp_to_analyze: str = None,
        dump_analysis_dir: str = None,
    ...
    def _read_logs_for_node(
        self, stateful_set_name: str, node_index: int, victoria_config_func
    ) -> List[pd.DataFrame]:
    ...
    def _read_logs_concurrently(self) -> List[pd.DataFrame]:
        ...
        for stateful_set_name, num_nodes_in_stateful_set in zip(
            self._stateful_sets, self._num_nodes
        ):
            with ProcessPoolExecutor(8) as executor:
                futures = {
                    executor.submit(
                        self._read_logs_for_node,
                        stateful_set_name,
                        node_index,
