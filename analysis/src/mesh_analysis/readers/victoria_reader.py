...
class NewVictoriaReader(Reader):
    """
    ...
    def __init__(
        ...
        """
        self._tracer: NewTracer = tracer
        self._config_query = victoria_config_query
        # self.extra_fields = extra_fields or []
        # logger.info(f"self.extra_fields: {self.extra_fields}")

    def _fetch_data(
        self, url: str, headers: Dict, params: Dict, extra_fields: List[str]
    ):
        logs = []
        ...
        with requests.post(
            ...
            for line in response.iter_lines():
                if line:
                    ...
                    logs.append(
                        (parsed_object["_msg"],)
                        + tuple(parsed_object[k] for k in self.extra_fields())
                    )
        logger.debug(f"Fetched {len(logs)} log lines")
