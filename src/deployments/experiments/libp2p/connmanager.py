...
class ConnManagerExperiment(BaseExperiment, BaseModel):
    ...
    async def _run(
        ...
        self._run_analysis(config, namespace, start_time, end_time, workdir)

    def _run_analysis(self, config, namespace, start_time, end_time, workdir):
        ...
        try:
            ...
            analyzer = (
                ConnManagerAnalyzer(
                    dump_analysis_dir=os.path.join(workdir, "analysis_data"),
                )
            ...
            out_dir = os.path.join(workdir, "plots")
