...
import numpy as np
import pandas as pd
from typing import Callable, Iterator, List, Self, Optional

from pydantic import BaseModel, Field

from src.mesh_analysis.readers.tracers.new_tracer import NewTracer, PatternGroup, TracePair
...
class NewWakuTracer(NewTracer):
    ...
    def with_wildcard_pattern(self) -> Self:
        self.patterns.append(
            PatternGroup(
                "wildcard",
                [TracePair(regex="(.*)", convert=self.trace_all_logs)],
                query="*",
            )
        )
        return self
    ...
    def trace(self, parsed_logs: List) -> dict:
        """
        ...
        """
        # expected_dim = (len(self.patterns), len(self.patterns[0].trace_pairs))

        def convert_logs(pattern_group, log_group):
            return [
                pattern_group.trace_pairs[i].convert(log)
                for i, log in enumerate(log_group)
            ]
    ...
    def _trace_legacy_lightpush_in_logs(self, parsed_logs: List) -> pd.DataFrame:
        ...
        parsed_logs = [
            [self.__class__.unknown_sender_str] + row for row in parsed_logs if row
        ]
        df = self._create_dataframe_with_timestamp(parsed_logs, columns)
    ...
    def _create_dataframe_with_timestamp(
        self, parsed_logs: List[str], columns: List[str]
    ):
        try:
