...
class MetricsPlotter:
    ...
    def _create_plot(self, plot_name: str, plot_specs: Dict):
        fig, axs = plt.subplots(
            nrows=1, ncols=len(plot_specs["data"]), sharey="row", figsize=plot_specs["fig_size"]
        )
    ...
    def _add_subplot_df_to_axs(
        ...
        result = add_boxplot_stat_labels(box_plot, scale_by=plot_specs.get("scale-x", 1))
        if result.is_err():
        ...
        if show_min_max:
            result = add_boxplot_stat_labels(
                box_plot, value_type="min", scale_by=plot_specs.get("scale-x", 1)
            )
            if result.is_err():
                logger.error(result.err_value)

            result = add_boxplot_stat_labels(
                box_plot, value_type="max", scale_by=plot_specs.get("scale-x", 1)
            )
            if result.is_err():
                logger.error(result.err_value)
