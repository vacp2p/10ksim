...
class NimMultipleRegression(BaseExperiment, BaseModel):
    ...
    def _run(
        ...
        for params in network_params:
            ...
            logger.info(
                f"UTC time: {this_time.hour:02d}:{this_time.minute:02d}"
            )
            ...
            exp_workdir = Path(workdir) / image / f"delay_{delay}_jitter_{jitter}_messages_{messages}_mix_{mix_nodes}_gossip_{nonmix_nodes}_rand_{random_number}"
            exp_args = deepcopy(args)
            exp_args.workdir = exp_workdir
            exp_args.delay = None
            exp_values_yaml = deepcopy(values_yaml)
            dict_set(exp_values_yaml, "nimlibp2p.nodes.network.delay", delay, sep=".", replace_leaf=True)
            dict_set(exp_values_yaml, "nimlibp2p.nodes.network.jitter", jitter, sep=".", replace_leaf=True)
            dict_set(exp_values_yaml, "nimlibp2p.nodes.env.vars.messages", messages, sep=".", replace_leaf=True)
            dict_set(exp_values_yaml, "nimlibp2p.nodes.env.vars.publishers", publishers, sep=".", replace_leaf=True)

            max_connections = params.get("maxConnections", None)
            if max_connections is not None:
                dict_set(exp_values_yaml, "nimlibp2p.nodes.env.vars.MAXCONNECTIONS", max_connections, sep=".", replace_leaf=True)

            msgRate = params.get("msgRate", None)
            if msgRate is not None:
                dict_set(exp_values_yaml, "nimlibp2p.nodes.env.vars.msgRate", msgRate, sep=".", replace_leaf=True)

            if mix_nodes is not None:
                dict_set(exp_values_yaml, "nimlibp2p.nodes.numMix", mix_nodes, sep=".", replace_leaf=True)
            if nonmix_nodes is not None:
                dict_set(exp_values_yaml, "nimlibp2p.nodes.numGossip", nonmix_nodes, sep=".", replace_leaf=True)
...
def get_batch() -> List[dict]:
    ...
    batch = []
    common = {'msgRate': 1000,'delay': 0, 'jitter': 0, 'numMix': 5, 'numGossip': 15, 'publishers': 5}
    updates = [
