...
class TestDataHandler(unittest.TestCase):
    ...
    def test_add_file_as_mean_to_df_with_data(self):
        ...
        dh = DataHandler()
        file_path_1 = Path(
            os.path.join(self.current_directory, "resources/test_folder_1/test1.csv")
        )
        file_path_2 = Path(
            os.path.join(self.current_directory, "resources/test_folder_1/test2.csv")
        )
    ...
    def test_dump_dataframe(self):
        ...
        data_handler = DataHandler()
        data_handler._dataframe = pd.DataFrame(
            {
                "A": [1, 2, 3, 4, 5],
                "B": [6, 7, 8, 9, 10],
