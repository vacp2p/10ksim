...
def dns_lookup(hostname, dns_servers):
    ...
    try:
        ...
        print(
            f"Successfully resolved {hostname} to {ip_address} using DNS server {target_dns or 'system default'}"
        )
        return True
    except Exception as e:  # Broad exception to catch all possible errors
        print(
            f"Failed to resolve {hostname} using DNS server {target_dns or 'system default'}: {e}"
        )
        return False
    finally:
        if target_dns:
...
def main(dns_servers):
    dns_servers_cycle = (
        cycle(dns_servers) if dns_servers else cycle([None])
    )  # Cycle through provided DNS servers or use default
    ...
    while True:  # Loop indefinitely
        for i in range(args.nodes):
            ...
            print(
                f"Processed {i + 1}/{args.nodes} hostnames, Success: {success_count}, Failures: {fail_count}"
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Perform DNS lookups with specified target DNS server or system default"
    )
    parser.add_argument(
        "--target", help="IP address of the target DNS server", nargs="*", default=[]
    )
    parser.add_argument(
        "--accept-input", action="store_true", help="Accept a list of IP addresses from user input"
    )
    ...
    if args.accept_input:
        input_servers = (
            input("Enter DNS IP addresses (comma or space-separated): ").replace(",", " ").split()
        )
