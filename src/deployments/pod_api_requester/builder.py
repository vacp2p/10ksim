...
class PodApiRequesterBuilder(PodBuilder):
    ...
    def _apply_requester_base_inner(
        ...
        if old_params and old_params.container_name:
            container = find_container_config(config, old_params.container_name)
        ...
        def command_from_mode(mode):
            if mode == "debug":
                return Command(command="sleep", args=["infinity"])
            else:
                return Command(
                    command="python",
                    args=[
    ...
    def _ensure_container(self):
        ...
        if not container_config:
            ...
            pod_config.add_container(
                ContainerConfig(name=self._container_name, image_pull_policy="Always")
            )
